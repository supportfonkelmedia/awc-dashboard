...
8. Revenue per LOB: validate `monthly_by_lob` is present in `revenue_per_lob` after deploy (omzet maandtrend per sub_administration).
9. Triple LOB (deel 2): validate `triple_lob.validation` distribution (zekerheid × aantal_lob) against Brief Fonkel — compare ratios, not exact counts. Expect ~7–9% klanten with concerns included.
